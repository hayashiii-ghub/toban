...
export function renderLandingPageHtml(origin: string): string {
  ...
  const desc = `学校・保育園・介護施設・自治会・オフィス・家庭の当番表をかんたんに作れる無料の当番表作成アプリ。用途別のテンプレートを${TEMPLATE_SEO_DATA.length}種類用意。アカウント登録・インストール不要、エクセルがなくてもブラウザだけで印刷品質の当番表がすぐ完成します。`;
